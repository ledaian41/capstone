 ...
 * @author Dell
 */
public class LoadInfoRegisterServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            String url = "register_pro.jsp";
            HttpSession session = request.getSession();
            CityJpaController cityJpa = new CityJpaController(emf);
            List<City> cities = cityJpa.findCityEntities();
            session.setAttribute("CITY", cities);
            TypeOfWorkJpaController workJpa = new TypeOfWorkJpaController(emf);
            List<TypeOfWork> works = workJpa.findTypeOfWorkEntities();
            session.setAttribute("WORK", works);
            request.getRequestDispatcher(url).forward(request, response);
        } catch (Exception e) {
            log("Error at LoadInfoRegisterServlet: " + e.getMessage());
        }
    }
